...
│   └── tui/                            # TUI E2E tests (NEW)
│       ├── test_workflow_spine.py      # Core workflow tests
│       ├── test_proposal_workflow.py   # Proposal lifecycle tests
│       ├── test_audit_fix_cycle.py     # Audit fix cycle tests
│       └── README.md                   # TUI E2E documentation
├── helpers/            # Test helper modules (NEW)
│   └── tui_automation.py               # TUI automation service
├── fixtures/           # Test fixtures and factories (NEW)
...
# TUI E2E tests only (NEW)
TERM=xterm-256color pytest tests/e2e/tui -v
...
5. **Proposal Workflow** _(disabled - router not yet enabled)_: Create → Apply → Verify
...
**Location**: `tests/e2e/tui/`

**Purpose**: Validate complete workflows through TUI client with automated, deterministic tests.

### Overview

The TUI E2E tests use automated command execution to test the full feature spine:

- **Workflow Spine**: Project creation → artifacts → proposals → audit
- **Proposal Workflow**: Manual and AI-assisted proposals, review, apply/reject
- **Audit Fix Cycle**: Audit → detect issues → fix → re-audit → verify clean
...
- **TUI Automation Service** (`tests/helpers/tui_automation.py`): Programmatic TUI command execution with timeout handling, JSON parsing, and performance measurement
- **Test Data Factories** (`tests/fixtures/factories.py`): Fluent builders for projects, artifacts, proposals, RAID items, audit results
- **Pytest Fixtures** (`tests/fixtures/fixtures.py`): Reusable fixtures for API server, TUI instances, test data

### Key Features

- ✅ **Deterministic execution**: No sleep-based timing, proper waits/retries
- ✅ **CI-integrated**: Runs in GitHub Actions with headless mode
- ✅ **Fast feedback**: Target < 5 minutes for full suite
- ✅ **Comprehensive coverage**: All Step 1 + Step 2 features

### Documentation

See [tests/e2e/tui/README.md](tests/e2e/tui/README.md) for:

- Detailed test scenario descriptions
- Test infrastructure documentation
- Writing new TUI E2E tests guide
- Debugging and troubleshooting tips
- Performance targets and CI integration

**Acceptance Criteria**:

- [ ] Workflow spine test passes (project → audit)
- [ ] Proposal workflow test passes (manual + AI)
- [ ] Audit fix cycle test passes (audit → fix → verify)
- [ ] Tests run deterministically (5 consecutive passes)
