...
def test_export_format_support():
    ...
    try:
        ...
        has_content = any(marker in content.lower() for marker in [
            'chat', 'conversation', 'issue', 'phase', 'step', 'user:', 'assistant:'
        ])
        
        if has_content:
            print(f"  ✅ Export format validated ({len(export_files)} files, {len(content):,} chars in sample)")
        else:
