...
def test_blueprint_creation_and_validation(client):
    ...
    assert response.status_code == 201, f"Failed to create PMP template: {response.text}"
    pmp_template_id = response.json()["id"]

    response = client.post("/api/v1/templates", json=raid_template)
    assert response.status_code == 201, f"Failed to create RAID template: {response.text}"
...
def test_artifact_generation_from_blueprint(client, test_project):
    ...
    for i in range(5):
        template_data = {
            ...
            "artifact_type": allowed_types[i % len(allowed_types)],  # Rotate through allowed types
            "version": "1.0.0",
        }
        response = client.post("/api/v1/templates", json=template_data)
        assert response.status_code == 201, f"Failed to create template {i}: {response.text}"
        template_id = response.json()["id"]
    ...
    for i, template_id in enumerate(templates):
        ...
        assert response.status_code == 201, f"Failed to generate artifact {i}: {response.text}"
...
def test_audit_events_validation(client, test_project):
    ...
    response = client.post(
        f"/api/v1/projects/{project_key}/raid", json=risk_data
    )
    ...
    assert isinstance(audit_data, dict) or isinstance(audit_data, list), "Audit data should be dict or list"
...
def test_error_handling(client, test_project):
    ...
    assert response.status_code in [404, 400], "Should return error for invalid template"
