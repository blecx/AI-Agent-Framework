...
class TestGUITutorial01WebInterface:
    ...
    def test_web_ui_loads(self, browser_page: Page):
        ...
        browser_page.wait_for_selector(
            "h1, [data-testid='app-title'], nav", timeout=15000
        )
    ...
    def test_api_docs_accessible(self, browser_page: Page):
        ...
        assert (
            "FastAPI" in browser_page.content()
            or "swagger" in browser_page.content().lower()
        )
...
class TestGUITutorial03CommandsProposals:
    ...
    def test_command_interface_exists(self, browser_page: Page):
        ...
        has_command_panel = (
            "CommandPanel" in page_content
            or "command" in page_content.lower()
            or "select" in page_content.lower()  # Command dropdown
        )
    ...
    def test_proposal_workflow_accessible(self, browser_page: Page):
        ...
        has_proposal_ui = (
            "ProposalModal" in page_content
            or "proposal" in page_content.lower()
            or "apply" in page_content.lower()
            or "reject" in page_content.lower()
        )
        ...
        assert (
            has_proposal_ui or buttons > 0
        ), "Proposal workflow UI should be accessible"
...
class TestGUITutorial04ArtifactBrowsing:
    ...
    def test_artifact_navigation_ui(self, browser_page: Page):
        ...
        has_artifact_ui = (
            "ArtifactsList" in page_content
            or "artifact" in page_content.lower()
            or "document" in page_content.lower()
            or "file" in page_content.lower()
        )
    ...
    def test_file_viewing_capability(self, browser_page: Page):
        ...
        clickable = browser_page.locator(
            "a, div[role='button'], [onclick], button"
        ).count()
        assert clickable > 0, "GUI should have file viewing/navigation elements"

        print(
            f"✅ File viewing capability present ({clickable} interactive elements found)"
        )
...
class TestGUITutorial05WorkflowStates:
    ...
    def test_workflow_state_display(self, browser_page: Page):
        ...
        has_workflow_ui = (
            "WorkflowIndicator" in page_content
            or "workflow" in page_content.lower()
            or "phase" in page_content.lower()
            or "initiation" in page_content.lower()
            or "planning" in page_content.lower()  # ISO 21500 phase
        )

        assert (
            has_workflow_ui
        ), "WorkflowIndicator component should display workflow state"
