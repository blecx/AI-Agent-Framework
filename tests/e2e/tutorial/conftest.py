...
class TUIAutomation:
    ...
    def add_raid_entry(self, project: str, entry_type: str, severity: str,
    ...
    def update_workflow(self, project: str, state: str):
        ...
        return self.run_command("workflow", "update", "--project", project, "--state", state)
    ...
    def apply_proposal(self, project: str, proposal_id: str):
        """Apply a proposal."""
        return self.run_command("proposals", "apply", "--project", project, "--id", proposal_id)
    
    def create_artifact(self, project: str, artifact_type: str, title: str, prompt: str):
        """Create an artifact."""
        return self.run_command(
            "artifacts", "create",
            "--project", project,
...
def pytest_configure(config):
    ...
    config.addinivalue_line(
        "markers", "tutorial: Tutorial E2E validation tests"
    )
    config.addinivalue_line(
        "markers", "tutorial_validation: Tutorial validation framework tests"
    )
    config.addinivalue_line(
        "markers", "tui: TUI-specific tests"
    )
    config.addinivalue_line(
        "markers", "gui: GUI-specific tests"
    )
    config.addinivalue_line(
