...
class TestTutorial02FirstProject:
    ...
    def cleanup(self, tui):
        ...
        # Clean before
        tui.execute_command(["projects", "delete", "--key", self.PROJECT_KEY], check=False)
        yield
        # Clean after
        tui.execute_command(["projects", "delete", "--key", self.PROJECT_KEY], check=False)
    ...
    def test_create_project(self, tui):
        ...
        result = tui.execute_command([
            ...
            "--description", "Test project for tutorial validation"
        ])
...
class TestTutorial03ArtifactWorkflow:
    ...
    def test_list_artifacts(self, tui):
        ...
        result = tui.execute_command(["artifacts", "list", "--project", self.PROJECT_KEY])
...
class TestTutorial05FullLifecycle:
    ...
    def test_workflow_update_phase(self, tui):
        ...
        result = tui.execute_command([
            ...
            "--project", self.PROJECT_KEY,
            "--phase", "planning",
            "--reason", "Test phase transition"
        ])

        # Workflow commands might not be fully implemented yet
        # So we check for success OR for a clear "not implemented" message
        if result.success:
            assert "planning" in result.stdout.lower() or "updated" in result.stdout.lower()
        else:
            # Command might not exist yet - that's OK for tutorials
            assert "not found" in result.stderr.lower() or "not implemented" in result.stderr.lower()
    ...
    def test_complete_lifecycle_scenario(self, tui):
        ...
        result = tui.execute_command(["projects", "state", "--project", self.PROJECT_KEY])
        assert result.success
