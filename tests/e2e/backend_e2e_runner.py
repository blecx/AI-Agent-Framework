...
def validate_backend(base_url):
    ...
    try:
        response = client.post(
            "/projects", json={"key": test_project_key, "name": "E2E Test Project"}
        )
...
Examples:
  ...
  python tests/e2e/backend_e2e_runner.py --mode wait-and-validate --url http://localhost:8000
    ...
    parser.add_argument(
        "--port", type=int, default=8000, help="Port to run server on (for server mode)"
    )
    parser.add_argument(
        "--docs-path", help="Path for project documents (temp dir if not specified)"
    )
    parser.add_argument(
        "--timeout", type=int, default=30, help="Timeout for wait operations in seconds"
    )
