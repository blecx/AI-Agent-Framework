...
class GitManager:
    ...
    def create_project(self, project_key: str, project_data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    def get_diff(self, project_key: str, file_path: str, content: str) -> str:
        ...
        if not full_path.exists():
            # New file
            lines = content.split('\n')
            diff_lines = ["--- /dev/null", f"+++ b/{project_key}/{file_path}", "@@ -0,0 +1," + str(len(lines)) + " @@"]
    ...
    def list_artifacts(self, project_key: str) -> List[Dict[str, Any]]:
        ...
        for file_path in artifacts_path.rglob("*"):
            if file_path.is_file():
                relative_path = file_path.relative_to(self.get_project_path(project_key))
                artifacts.append({
    ...
    def get_last_commit(self, project_key: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            commits = list(self.repo.iter_commits(paths=str(self.get_project_path(project_key))))
            if commits:
                commit = commits[0]
                return {
                    "hash": commit.hexsha,
