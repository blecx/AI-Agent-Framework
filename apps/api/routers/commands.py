...
async def propose_command(
    project_key: str,
    command_request: CommandPropose,
    request: Request
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to propose command: {str(e)}")


@router.post("/apply", response_model=CommandApplyResult)
async def apply_command(
    project_key: str,
    apply_request: CommandApply,
    request: Request
):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to apply command: {str(e)}")
